...
test('derives the 9:20 safe margins and quote box from the requested pixels', () => {
  ...
  expect(composition.quoteBounds.y).toBeCloseTo(916.92);
  expect(composition.quoteBounds.width).toBeCloseTo(907.2);
  expect(composition.quoteBounds.height).toBeCloseTo(182.16);
  expect(composition.authorY).toBeCloseTo(1151.88);
  expect(composition.quoteFontSize).toBe(66);
...
test('keeps every alignment and stress length in the 1080x2400 canonical safe box', () => {
  ...
  // No shipped preset is right-aligned, but the renderer still draws that
  // alignment, so every preset is measured under all three rather than only
  // under the ones the catalogue happens to hold.
  for (const current of getAllPresets()) {
    for (const length of lengths) {
      for (const textAlign of textAlignments) {
        const composition = createComposition({
          quote: quoteOfLength(length),
          preset: { ...current, textAlign },
          width: 1080,
          height: 2400,
          locale: 'en',
        });
...
test('creates a stable cache key for the same quote, preset, and dimensions', () => {
  ...
  expect(first.cacheKey).toMatch(
    /^violet-growth-quote-80-1080x2400-[0-9a-z]+$/,
  );
