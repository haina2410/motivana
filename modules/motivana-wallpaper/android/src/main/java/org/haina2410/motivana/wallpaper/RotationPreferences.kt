...
class RotationPreferences(context: Context) {
  ...
  fun rawSnapshot(): String? = prefs.getString("snapshot", null)
  fun rawStatus(): String? = prefs.getString("status", null)
  ...
  fun restoreRawSnapshot(value: String?): Boolean = prefs.edit().also { if (value == null) it.remove("snapshot") else it.putString("snapshot", value) }.commit()
  fun restoreRawStatus(value: String?): Boolean = prefs.edit().also { if (value == null) it.remove("status") else it.putString("status", value) }.commit()
}
